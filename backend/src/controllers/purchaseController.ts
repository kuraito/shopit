...
export const getPurchases = async (req: Request, res: Response) => {
  try {
    ...
    const decoded: any = jwt.verify(token, SECRET) //verifica il token e decodifica, se non è valido lancia un errore
    const userId = decoded.id //prendi l'id dell'utente dal token decodificato

    // Prendi gli acquisti dell'utente
    const [rows]: any = await pool.query(
      `SELECT p.id, p2.description, p.price_snapshot, p.created_at, p2.name
...
export const createPurchase = async (req: Request, res: Response) => {
  try {    
    ...
    await pool.query(
      "INSERT INTO purchase (user_id, product_id, price_snapshot, created_at) VALUES (?, ?, ?, NOW())",
